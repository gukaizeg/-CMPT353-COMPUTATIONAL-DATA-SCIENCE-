...
def to_lab(color):
    return rgb2lab(np.array([[color]]))[0][0]

def to_hsv(color):
    return rgb2hsv(np.array([[color]]))[0][0]

bayes_rgb_model = GaussianNB()
bayes_convert_model = make_pipeline(FunctionTransformer(to_lab, validate=True), GaussianNB())
knn_rgb_model = KNeighborsClassifier()
knn_convert_model = make_pipeline(FunctionTransformer(to_lab, validate=True), KNeighborsClassifier())
rf_rgb_model = RandomForestClassifier()
rf_convert_model = make_pipeline(FunctionTransformer(to_hsv, validate=True), RandomForestClassifier())
