...
def get_data(filename):
    ...
    # Add the column that we want to predict: the temperatures from the *next* time step.
    sysinfo[y_column] = sysinfo['temperature'].shift(-1)
    sysinfo = sysinfo.dropna(subset=[y_column])  # Drop rows where the next temperature is missing (last row)
...
def smooth_test(coef, sysinfo, outfile):
    ...
    transition[0] = coef  # Replace the first row of the transition matrix with the calculated coefficients
